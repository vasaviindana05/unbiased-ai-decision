            ...
            with st.spinner("Gemini AI is analyzing your dataset..."):
                try:
                    ...
                    if "403" in err_str or "API_KEY_INVALID" in err_str or "invalid" in err_str.lower():
                        st.warning("🔑 Your API key is invalid or revoked. Generate a new one at https://aistudio.google.com/apikey and update Streamlit Secrets.")
                    elif "429" in err_str or "quota" in err_str.lower():
                        st.warning("⏳ Rate limit or quota exceeded. Wait a moment and try again, or check https://aistudio.google.com.")
                    elif "401" in err_str:
                    ...
                    # Key preview for debugging
                    try:
                        key_preview = st.secrets["GEMINI_API_KEY"][:10] + "..."
                        st.info(f"🔍 Key loaded (first 10 chars): `{key_preview}`")
                    except Exception:
                        st.error("❌ GEMINI_API_KEY not found in secrets at all!")
